...
* The file `rules.json` has been replaced by `core/category_rules_regex.json`.
* The categorization rules (keys) inside `core/category_rules_regex.json` must be dynamically named after the transaction types found in the **"Backend Data"** tab under the **"Transaction Type"** column.
...
2. **Transaction Types:** Fetch the transaction types locally from the `core/category_rules_regex.json` file (where the keys represent the valid types). Do not attempt to hit the Google Sheets API.
...
2. **Transaction Rules Sync:**
   ...
   * Create or update the local `core/category_rules_regex.json` file.
